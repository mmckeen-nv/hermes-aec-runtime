...
_VERBS = {
    ...
    Intent.VISUALIZE: {"render", "camera", "lighting", "visualize", "material", "stylize", "comfyui"},
...
def route_request(request: str, *, active_host: str = "rhino") -> RequestRoute:
    ...
    word_set = set(words)
    wants_comfyui = bool(word_set & {"comfyui", "stylize", "stylized", "diffusion", "flux"})
    ...
    if not word_set & _VERBS[Intent.RECOVER] and active_host.casefold() != "blender" and "blender" in word_set and word_set & _VERBS[Intent.HANDOFF]:
    ...
    if intent is Intent.RECOVER:
        ...
        tools = (f"{source_host}_scene_query",) + export_tools + ("blender_validate_handoff", "blender_import_handoff", "blender_scene_query", "blender_apply_operations", "blender_proof_and_recovery")
        if wants_comfyui:
